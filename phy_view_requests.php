...
$sql = "SELECT * FROM appointment JOIN userdata phy ON appointment.physician_id=phy.user_id JOIN userdata pat ON appointment.patient_id = pat.user_id WHERE physician_id=".$physician_id." AND approval='Pending' AND status='Ongoing'";
    ...
    <section class="sidebar">
      ...
      <ul class="sidebar-menu">
        ...
        <li class="active">
          ...
          <ul class="treeview-menu">
            <li class="active"><a href="phy_view_requests.php"><i class="fa fa-question"></i> View Requests</a></li>
                      ...
                      if($result->num_rows > 0){
                        while($row = $result->fetch_array()){
                          ...
                          <tr>
                           ...
                           <td>
                             <center>
                               <button type='button' class='btn btn-success approve' id='".$row['appointment_id']."'><i class='fa fa-check'></i> Approve</button>
                               <button type='button' class='btn btn-danger deny' id='".$row['appointment_id']."'><i class='fa fa-times'></i> Deny</button>
                             </center>  
                           </td>
      ...
      <div class='modal fade' id='approve' tabindex='-1' role='dialog'>
       <div style='background-color: white' class='modal-dialog modal-md'> 
        <div class='modal-editMedicine'>
          <div class='modal-header'>
           <button type='button' class='close' data-dismiss='modal' aria-hidden='true'>&times;</button>
           <h4 class='modal-title' id='myModalLabel'>Approve Appointment</h4>
         </div>
        ...
        <div class='modal-footer'>
          <button type='button' class='btn btn-default' data-dismiss='modal' aria-hidden='true'><i class='fa fa-undo'></i> Cancel</button>
          <button type='button' class='btn btn-success confirm-approve' aria-hidden='true'><i class='fa fa-check'></i> Approve</button>
        </div>
            </div>
          </div>
        </div>
      ...
      <div class='modal fade' id='deny' tabindex='-1' role='dialog'>
       <div style='background-color: white' class='modal-dialog modal-md'> 
        <div class='modal-editMedicine'>
          <div class='modal-header'>
           <button type='button' class='close' data-dismiss='modal' aria-hidden='true'>&times;</button>
           <h4 class='modal-title' id='myModalLabel'>Deny Appointment</h4>
         </div>

         <div class='modal-body'>
          <div class='pad' id='infopanel'></div>                                
          <div class='form-horizontal' style="margin-top: -20px">
            <h5> Are you sure you want to deny this appointment? </h5>
          </div>                  
        </div>
        <div class='modal-footer'>
          <button type='button' class='btn btn-default' data-dismiss='modal' aria-hidden='true'><i class='fa fa-undo'></i> Cancel</button>
          <button type='button' class='btn btn-danger confirm-deny' aria-hidden='true'><i class='fa fa-times'></i> Deny</button>
        </div>
            </div>
          </div>
        </div>
  ...
  $(document).ready(function(){
    ...
    $(document).on('click', '.approve', function(){      
        ...
        $('.confirm-approve').on('click', function() {
            $.ajax({
                url:"phy_approve_appointment.php",
                method: "POST",
                data:{appointment_id:appointment_id},
                success:function(data){
                  if(data == 1){
                    sessionStorage.setItem('approvalKey', 1);
                    window.location = "http://localhost/MedicalSolution/phy_appointment_records.php";  
                  }
                }
            });
        });
    ...
    $(document).on('click', '.deny', function(){      
        $('#deny').modal('show');
        var appointment_id = $(this).attr("id");
        $('.confirm-deny').on('click', function() {
            $.ajax({
                url:"phy_deny_appointment.php",
                method: "POST",
                data:{appointment_id:appointment_id},
                success:function(data){
                  if(data == 1){
    ...
    if(sessionStorage.getItem('denyKey') == 1){
      $.notify({
              // options
              icon: 'glyphicon glyphicon-ok',
              message: ' Appointment has been successfully denied.' 
            },{
              // settings
              type: 'success'
            });
...
<script type="text/javascript">
  //Script for VIEWING appointment's details inside modal
  $('.appointment-view').click(function(){
    var appointment_id = $(this).attr('data-id');
    $.ajax({url:"phy_requests_modal.php?appointment_id="+appointment_id,cache:false,success:function(result){
