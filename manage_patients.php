...
$sql = "SELECT * FROM userdata JOIN patient ON patient.patient_id=userdata.user_id";
...
<body class="hold-transition skin-blue sidebar-mini">
  <div class="wrapper">
          ...
          <ul class="sidebar-menu">
            ...
            <li class="treeview active">
              ...
              <ul class="active treeview-menu">
                ...
                <li class="active"><a href="manage_patients.php"><i class="fa fa-user"></i> Patients</a></li>
              </ul>
            </li>
            ...
            <div class="col-xs-12">           
              ...
              <div class="box">
                <div class="box-header">
                  <h3 class="box-title">Patient's Masterlist</h3>
                </div>
                <div class="box-body">
                  <button type="button" class="btn btn-primary" id="btnadd" name="btnadd" data-toggle="modal" data-target="#newPatient"><i class="fa fa-plus"></i> Add Patient</button>                
                      ...
                      if($result->num_rows > 0){
                        while($row = $result->fetch_assoc()){
                          ...
                          <tr>
                           ...
                           <td><a href='#' class='patient-view' data-toggle='modal' data-target='#viewPatientPersonal' data-id='".$row['user_id']."'>".$row['firstname']." ".$row['lastname']."</a></td>
                           <td><center>
                            <button type='button' class='btn btn-primary check-health' data-toggle='modal' data-target='#viewPatientHealth' data-id='".$row['user_id']."'><i class='fa fa-heartbeat' aria-hidden='true'></i> View Patient Status </button>
                          </center>
                        </td>

                        <td>
                         <center>
                           <button type='button' class='btn btn-info edit_patient' id='".$row['user_id']."'><i class='fa fa-pencil-square-o'></i> Edit</button>
                           <button type='button' class='btn btn-danger delete' id='".$row['user_id']."'><i class='fa fa-trash-o'></i> Delete</button>
                         </center>  
                       </td>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="form-horizontal">
              <form method="POST" action="" id="new_patient">
                ...
                <h4 style="font-weight:bold; margin-top:40px"> Health Information</h4>
                <hr>
                <div class="form-group"> 
                  <label class="col-sm-3 control-label">Weight</label>
                  <div class="col-sm-9">
                    <input type="number" class="form-control" name="weight" id="weight" placeholder="In kilograms" required>
                  </div>
                </div>
                ...
                <div class="form-group"> 
                  ...
                  <div class="col-sm-9">
                    <input type="number" class="form-control" name="height" id="height" placeholder="In centimeters" required>
                  </div>
                </div>
                ...
                <div class="form-group"> 
                  ...
                  <div class="col-sm-9">
                    <input type="text" class="form-control" name="bloodPressure" id="bloodPressure" required>
                  </div>
                </div>
    ...
    <!--/Edit Patient Modal-->
    <div class='modal fade' id='deletePatient' tabindex='-1' role='dialog'>
     <div style='background-color: white' class='modal-dialog modal-md'> 
      <div class='modal-editPatient'>
        <div class='modal-header'>
...
<!--View Patient Modal - Personal-->
<div class="modal fade" id="viewPatientPersonal" tabindex="-1" role="dialog">
  <div class="modal-dialog modal-md">
    <div class="modal-viewPatientPersonal">

    </div>
  </div>
</div>

<!--View Patient Modal - Health-->
<div class="modal fade" id="viewPatientHealth" tabindex="-1" role="dialog">
  <div class="modal-dialog modal-md">
    <div class="modal-viewPatientHealth">
...
<script>
  ...
  $(document).ready(function(){
    $('#newPatient').on('hidden.bs.modal', function (e) {
      $(this)
    ...
    $('#new_patient').on('submit',function(event){      
      event.preventDefault();
      $.ajax({
        url:"new_patient.php",
        method:"POST",
        data:$('#new_patient').serialize(),
        success: function(data)
        { 
          if(data == 1){
          ...
          if(data == 2){
            $.notify({
              ...
              message: ' Patient could not be inserted. Check your queries.' 
            },{
                  // settings
                  type: 'danger',
                  z_index: 6000,
          ...
          if(data == 4){
            $.notify({
              // options
              icon: 'glyphicon glyphicon-ban-circle',
              message: ' Patient could not be updated. Check your queries.' 
            },{
                  // settings
                  type: 'danger',
                  z_index: 6000,
    ...
    if(sessionStorage.getItem('key') == 1){
      $.notify({
              ...
              message: ' Patient has been successfully added to the database.' 
            },{
              // settings
              type: 'success'
            });
    ...
    if(sessionStorage.getItem('key') == "Delete"){
      $.notify({
              // options
              icon: 'glyphicon glyphicon-ok',
              message: ' Patient has been successfully deleted in the database.' 
            },{
              // settings
              type: 'success'
            });
    ...
    if(sessionStorage.getItem('key') == "Record Found"){
      $.notify({
              // options
              icon: 'glyphicon glyphicon-ban-circle',
              message: ' Notice: Patient has existing records in the system.' 
            },{
              // settings
              type: 'danger'
            });
...
<script type="text/javascript">
  ...
  $('.patient-view').click(function(){
    var patient_id = $(this).attr('data-id');
    $.ajax({url:"view_patient_modal.php?patient_id="+patient_id,cache:false,success:function(result){
      $(".modal-viewPatientPersonal").html(result);
    }});
  });

  $('.check-health').click(function(){
    var patient_id = $(this).attr('data-id');
    $.ajax({url:"view_patientHealth_modal.php?patient_id="+patient_id,cache:false,success:function(result){
      $(".modal-viewPatientHealth").html(result);
    }});
  });
