...
if(!empty($_POST)){
	...
	$update_appointment = "UPDATE appointment SET status='Done' WHERE appointment_id=".$appointment_id;
	$result_update_appointment = mysqli_query($conn, $update_appointment);

	if(!$result_visit && !$result_checkup && !$result_update && !$result_presc && !$result_update_appointment){
		echo 0;
	}else{
		echo 1;
	}
